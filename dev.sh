...
workon teste  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development teste npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop teste
    docker rm teste
    docker run --name teste -d --env-file /home/ubuntu/teste.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/teste:/dkdata \
        teste start_web.sh
}
...
echo_green "Welcome to teste's dev env"
